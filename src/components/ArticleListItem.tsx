...
const ArticleListItem = ({data: {id, title, summary, link, time, tags = [], feed, box, priority, series = {}}, onClickItem, match: {params: {box: route_box = "all"}}, location: {search},}: any) => {
    const {id: seriesId, title: seriesTitle}: any = series || {};
    const {mode: viewMode} = useContext(ViewModeContext);
    const {t} = useTranslation("", {useSuspense: false});
    let {read = "all"}: any = queryString.parse(search);
    ...
    return (<div>
        <Paper elevation={1} className={styles.container} style={{opacity: days2opacity(daysFromNow(time))}} key={id}>
            ...
            <div className={styles.foot}>
                ...
                <Typography className={styles.tags}>{t('tags')}:{tags.map((it: any) => <span key={it} className={styles.tag}><Link to={`/article/tag/${it}`}>{it}</Link></span>)}</Typography>
                <Typography>{t('feed')}: {feed_title || feed_link || ''}</Typography>
                {seriesId ? <Typography component={props => <Link to={`/series/${seriesId}`}/>}>{t('Series')}: {seriesTitle || seriesId || ''}</Typography> : null}
            </div>
        </Paper>
    </div>);
};
