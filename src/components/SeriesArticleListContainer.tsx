...
const query = gql`query series_articles($tag:ID!,$cursor: String,$read:String="all",$priority:Int) {
    node(id:$tag,type:"Series"){
        id
        ... on Series{
            name:title
            articles(page:{last:10,before: $cursor},box:"all",read:$read,priority:$priority) {
                pageInfo{
                    startCursor
                    endCursor
                    hasNextPage
...
${fragment_article_list_item}
`;
const TopicArticleListContainer = (props: any) => {
    const {location: {search}, match: {params: {tag}}} = props;
    let {read = "unread", priority = "0"}: any = queryString.parse(search);
...
const ArticlePriorityList = ({location: {search}, match: {params: {tag}}, priority = "0"}: any) => {
